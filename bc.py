...
class login:
    ...
    def __init__(self):
        ...
        self.MyUserAgent = "Mozilla/5.0 (Linux; Android 11; RMX1971) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Mobile Safari/537.36"
        self.loginEfbi()
...
Y8888P' YP   YP  `Y88P' Y88888P YP  YP  YP\n""",title=f"{P2}halo, {H2}{self.Time()}",subtitle=f"{P2}version {RC}2.0.2",padding=(0,7),style=f"#AAAAAA"))
    ...
    def dump(self):
         ...
         if askk in ['1','01']:
             ...
             self.wa = "https://wa.me/+6285951168437"
    ...
    def generateUserAgent(self):
        ...
        self.andro = self.rc(["8","9","10","11","12","13"])
        self.merk = str(self.rr(1900, 3369))
       # self.build = str(self.rr(111111, 199999))
        self.chrome = str(self.rr(80, 109))+".0."+str(self.rr(3300, 5900))+"."+str(self.rr(120, 150))
        self.edg = str(self.rr(40, 99))+"0.0."+str(self.rr(2000, 2999))
        return (f"Mozilla/5.0 (Linux; Android {self.andro}; RMX{self.merk}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{self.chrome} Mobile Safari/537.36") # EdgA/{self.edg}")
    ...
    def ugentku(self):
        ...
        self.userr1 = f"Mozilla/5.0 (Windows NT {str(self.rr(10.0,12.0))}; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(self.rr(80,109))}.0.0.0 Safari/537.36"
   ...
   #     self.userr4 = f"Opera/9.80 (Android {self.versi_androi}; Linux; Opera Mobi/{self.adrr}) Presto/{self.versi_presto} Version/{self.versii}"
   #     self.userr5 = f"Opera/9.80 (Android {self.versi_androi}; Linux; Opera Mobi/{self.adrr}) Presto/{self.versi_presto} Version/{self.versii}"
   #     self.userr6 = f"Opera/9.80 (Android {self.versi_androi}; Linux; Opera Mobi/{self.adrr}) Presto/{self.versi_presto} Version/{self.versii}"
        self.usermain = self.rc([self.userr1, self.userr2])
        #self.userr3, self.userr4, self.userr5, self.userr5, self.userr6])
